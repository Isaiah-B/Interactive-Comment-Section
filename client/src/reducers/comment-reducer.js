...
const commentSlice = createSlice({
  ...
  reducers: {
    ...
    updateParent(state, action) {
      ...
      const parentComment = current(state).find((comment) => comment._id === parentId);

      const parentCopy = { ...parentComment };
...
export const createComment = (content, token) => async (dispatch) => {
...
export const createReply = (content, parentId, token) => async (dispatch) => {
  commentService.setToken(token);
...
export const editComment = (comment, updatedFields, token) => async (dispatch) => {
  commentService.setToken(token);
  const updatedComment = await commentService.updateComment(comment._id, updatedFields);
  dispatch(updateComment(updatedComment));
};

export const deleteComment = (comment, token) => async (dispatch) => {
  commentService.setToken(token);
